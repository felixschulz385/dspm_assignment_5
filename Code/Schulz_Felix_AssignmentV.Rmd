...
I hereby declare that this work and the code is solely my work. I did not work
as part of any group for this assignment.
...
Testing this function, I call it to retrieve all German data. The resulting
data frame turns out much smaller than the example from the assignment's 
instructions.
...
Since 
`r (venue_data %>% filter(is.na(latitude) | is.na(longitude)) %>% nrow() / venue_data %>% nrow()) %>% round(0) %>% "*"(100)` 
of all venues do not have coordinates assigned to them, one should display them
by their postal code to grasp the entirety of the data. 

First, I create simplified versions of openly available data that I make 
accessible for download for replication purposes. This reduces the required
download from 100mB to less than 400kB. For future re-runs of
this code, the simplifying is therefore not run by default but the resulting
data is directly downloaded.
...
I believe that displaying the data on the postcode level is not feasible and
does not produce easily comprehensible results. Therefore, I aggregate the data
on the county level. I use an open-source database retrieved from 
[opendatasoft.com](https://public.opendatasoft.com/explore/dataset/georef-germany-postleitzahl/information/)
to create a mapping of postcodes to counties. Since the data is licensed under
the *Open Database License*, I am able to modify and share the data.
...
Both resulting files are zipped and uploaded to a cloud hosting service.
...
read_sf("Data/georef-germany-kreis") %>%
  # Select a subset of columns
...
parameter of the *countryCode*. There are 1,017 venues listed on 
ticketmaster.com across the Netherlands.

```{r, message = F, warning = F}
venue_data_nl = 
  ticketmaster_GET_all_venues(list("countryCode" = "NL"), api_key)
  
glimpse(venue_data_nl)
```

The map can also largely be reproduced using the code from above. The most
