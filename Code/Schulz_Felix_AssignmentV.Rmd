...
I also set up lists to contain API responses.
...
ticketmaster_GET_all_venues = function(params = NULL, api_key){
  ...
  tmp[[i]] = 
    ...
    # Select the required columns
    select(name = "name...1", 
           city = colnames(.)[str_detect(colnames(.), "name...") & colnames(.) != "name...1"], 
...
Since 
`r venue_data %>% filter(is.na(latitude) | is.na(longitude)) %>% nrow() / venue_data %>% nrow()` 
...
ggplot() + 
  ...
  geom_point(aes(x = longitude, y = latitude),
             data = 
               venue_data_nl %>%
               filter(!is.na(longitude) & !is.na(latitude),
                      longitude > map_data("world", region = "Netherlands") %>% pull(long) %>% min() & 
                        longitude < map_data("world", region = "Netherlands") %>% pull(long) %>% max(),
                      latitude > map_data("world", region = "Netherlands") %>% pull(lat) %>% min() & 
                        latitude < map_data("world", region = "Netherlands") %>% pull(lat) %>% max())) +
  theme_void() + 
  coord_quickmap() +
  labs(title = "Event locations across Germany", caption = "Source: ticketmaster.com") + 
  theme(title = element_text(size=8, face='bold'),
